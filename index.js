...
var Dialog = ( module.exports = {
  ...
  show: function( type, str, title, callback ) {
    if ( !str || str.trim() == '' ) throw new Error( 'Empty or no string passed!' );
    ...
    // return codes for zenity are 0 on "OK" and 1 on "No/Cancel" or close window
    if ( os_name == 'linux' ) {
      str = str.replace( /[<>]/g, '' );
      cmd.push( 'zenity' );
      cmd.push( '--' + type );
      ...
      // return codes in macOS are exactly as in Linux 0 for 'OK', 1 for 'Cancel'
    }
      ...
      // return codes for Windows (default value minus 1) defaults in
